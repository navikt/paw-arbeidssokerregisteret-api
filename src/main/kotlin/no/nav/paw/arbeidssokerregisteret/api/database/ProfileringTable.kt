...
object ProfileringTable : LongIdTable("profilering") {
    ...
    val opplysningerOmArbeidssoekerId = uuid("opplysninger_om_arbeidssoeker_id").references(OpplysningerOmArbeidssoekerTable.opplysningerOmArbeidssoekerId)
    val sendtInnAvId = long("sendt_inn_av_id").references(MetadataTable.id)
    val profilertTil = customEnumeration("profilert_til", "ProfilertTil", { value -> ProfilertTil.valueOf(value as String) }, { PGEnum("ProfilertTil", it) })
